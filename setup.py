...
def _get_cudss_lib_dir():
    """Find CUDSS library directory for DLL/SO bundling."""
    if sys.platform == 'win32':
        if CUDSS_DIR:
            # CUDSS installed version has DLLs in bin/12 or bin/13
            # CUDSS redistributable archive has DLLs directly in bin/
            for cuda_ver in ['13', '12']:
                bin_dir = os.path.join(CUDSS_DIR, 'bin', cuda_ver)
                if os.path.exists(bin_dir):
                    return bin_dir
            # Fall back to bin/ directly (redistributable structure)
            bin_dir = os.path.join(CUDSS_DIR, 'bin')
            if os.path.exists(bin_dir):
                return bin_dir
    else:
        # Linux
